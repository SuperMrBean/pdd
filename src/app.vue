                ...
                <el-row style="display:flex;align-items:center">
                  ...
                  <el-col :span="5">
                    <div class="address" v-if="tradeIndex === 0">
                      <div>{{ item.receiverInfo.receiverName }}</div>
                      <div>{{ item.receiverInfo.receiverMobile }}</div>
                      <div>
                        {{
...
export default {
  ...
  methods: {
      ...
      proxy({
        ...
        onResponse: (response, handler) => {
          ...
          if (url.indexOf("/trade/details") > -1) {
            ...
            const list = content.reduce((acc, cur) => {
              const {
                ...
                receiver_state = "",
                receiver_town = "",
                receiver_mobile_mask = "",
                receiver_name_mask = "",
                receiver_mobile_encrypt = "",
                tid = "",
                orders = [],
                status = "",
              } = cur || {};
          ...
          if (url.indexOf("/trade-print/get-seller-flag-by-tids") > -1) {
            const flagInfo = JSON.parse(data) || [];
          ...
          if (url.indexOf("/external-authority/find-by-shop-id") > -1) {
            const { externalUsername = "" } = JSON.parse(data) || {};
            this.userInfo.externalUsername = externalUsername;
            if (this.$root.token) {
    ...
    onCheckName(userInfo, shopInfo, token) {
      const { externalUsername = "" } = userInfo;
